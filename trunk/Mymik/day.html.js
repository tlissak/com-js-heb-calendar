...
Day.prototype.output_html=function(){
	...
	chashashot_html+="<div onClick=\"cal._selected_shape='diamond';\" style=\"position:absolute;left: 39px; top: 47px; width: 19px; height: 19px; background:URL('./images/diamond.png')\"></div>";
		...
		chashashot_html+="<div onClick=\"cal._selected_shape='square_left';\" style='background: transparent url(./images/2digits.png); font-size: 11px; font-family:Verdana; position: absolute; left: 12px; top: 60px; width: 17px; height: 15px;'>"+this._events[i]._misc+"</div>";
		else if(this._events[i]._onah==_DAY_&&this.show_chashashot(_DAY_))
		...
		{if(this._events[i]._onah==_NIGHT_&&this.show_chashashot(_NIGHT_))
		chashashot_html+="<div onClick=\"cal._selected_shape='square_left';\" style='background: transparent url(./images/3digits.png); font-size: 11px; font-family: Verdana; position: absolute; left: 9px; top: 60px; width: 28px; height: 15px;'>"+this._events[i]._misc+"</div>";
		else if(this._events[i]._onah==_DAY_&&this.show_chashashot(_DAY_))
		...
		var night_div="<div style=' font-size:16px;position:absolute; left:0px; top:45px; width:48px; height:21px;' align='center'><font face='Geneva'  color='#666666'>";var day_div="<div id='ht' style=' font-size:16px;position:absolute; left:48px; top:45px; width:48px; height:21px;' align='center'><font face='Geneva' color='#666666'>";
		if(this._night_color==_BLUE_&&this._mikvah!==undefined)
		{dow="<div style='font-size:9px;color:#555555;line-height: 11px;'>"+getLongWeekdayName(this._mikvah._mikvah.get_dow()-2)+" Night</div>";
		if(this._mikvah.check_for_bad_mikvah()=='')
		...
		text="B";
		else if(!this._reeyah.goesOnCalendar())
		...
		chashashot_html+="<div style=\"position:absolute; left: 29px; top: 34px; width: 37px; height: 39px;background:URL('./images/plane3.png')\"></div>";
		var icons_night='';var icons_day='';for(i in this._memos)
		if(this._memos[i][0]._onah==_NIGHT_)
		icons_night="<div onClick=\"cal._selected_shape='memo';\" style='background: transparent url(./images/days/memo.jpg); position: absolute; left: 12px; top: 3px; width: 12px; height: 10px;'></div>";
		else if(this._memos[i][0]._onah==_DAY_)
		icons_day="<div onClick=\"cal._selected_shape='memo';\" style='background: transparent url(./images/days/memo.jpg); position: absolute; left: 70px; top: 3px; width: 12px; height: 10px;'></div>";
		var output="<td id = 'day_"+this._date._d+"' onMouseOut=\"this.style.cursor='default'\" onMouseOver=\"this.style.cursor='pointer'\" onClick='top.frames[1].cal._selected_day = "+this._date._d;
		output+="+ top.frames[1].cal._selected_offset;top.frames[1].refresh();if(top.frames[1].cal._selected_offset != 770){top.frames[1].new_popup_win_fromString();}'  height='90px' valign='top' style='background:url(./images/days/"+this.get_bg_image()+"); background-repeat:repeat-y; padding-top:4px; padding-bottom:4px'>";
		output+="<div align='center' style=';position:relative; width:95px; height:90px'>";
		output+="<table style=\"background:url(\'./images/day_bg.png\') repeat; border: 1px solid #999999;border-collapse: collapse;\"   width='76'";
		output+="border='0' align='center' cellpadding='0' cellspacing='0' >";
		output+="<tr align='center'>";output+="<td><div align='center' style=\"display:inline;font-size: 12px;line-height:15px; font:'Lucida Sans Unicode'\">"
		output+=this._date._d+"</div><div align='center' style=\"display:inline;padding-left:3px;font-size: 14px;line-height:13px; font:'Lucida Sans Unicode'\">"+this.alefbeis_day()+"</div></td>";output+="</tr><tr><td>";
		if(this._chag.length>13)
