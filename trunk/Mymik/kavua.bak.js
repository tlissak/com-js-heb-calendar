...
function find_kavuah(veses,cal){
	...
	for(i in veses._haflagas)
		if(veses._haflagas[i][1]>2&&veses._haflagas[i][2]==veses){
			kavuah_text+="A Fixed cycle (veses kavuah) has been  established by three flows all having haflagah number "
			+veses._haflagas[i][0];kavuah_text+=".<br><br>";result=true;}
	if(result){
...
function find_sirug(veses, cal) {
   ...
   for(i in cal._veses) if(!cal._veses[i]._kessem) reeyahs.push(new Array(cal._veses[i]._reeyah, cal._veses[i]._reeyah.get_dow(), i)); 
   ...
   for(pattern_length = 1; pattern_length <= max_pattern_length; pattern_length++) {
      ...
      if(!failed) {
         ...
         if(pattern_length == 1) {
            ...
            for(x = pattern_length - 2; x > 0; x--) pattern_str += ", " + gaps[x]; 
         ...
         if(pattern_length == 1) {
            ...
            if(different_days) {
               ...
               if(good_pattern) {
                  ...
                  for(i in show_onahs_for) if(i == pattern_length - 1) {
...
function find_chodesh_kavuah(veses,cal){
	...
	if(cal._veses[j]._reeyah.equals(r)&&cal._veses[j]._onah==veses._onah){
		...
		kavuah_reeyahs[1]=veses;return true;}}
		return false;
	};// JavaScript Document