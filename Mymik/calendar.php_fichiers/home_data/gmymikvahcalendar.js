...
HDate.prototype._d;HDate.prototype._m;HDate.prototype._y;
function HDate_from_english(eng_d,eng_m,eng_y,hour,minutes,location){
	hebrewFromAbsolute(absoluteFromGregorian(eng_d,eng_m+1,eng_y));
	var hebrew_date=new HDate(returnDateDay,returnDateMonth,returnDateYear);
	var hebrew_day=new Day(hebrew_date);
	var time=(hour*60)+minutes;var end_shkiah;
	if(location!=undefined){end_shkiah=get_zman(hebrew_day,3,-1,location);}
	elseend_shkiah=hebrew_day.end_shkiah();
	if(time>=end_shkiah)
		hebrew_date.next();
	return hebrew_date;
}
...
HDate.prototype.isShabbosOrYontef=function(){
	if(this.get_dow()==7) return true;
	if(this.equals(getJewishHolidayDate(0,this._y))||this.equals(getJewishHolidayDate(1,this._y))||this.equals(getJewishHolidayDate(2,this._y))
		||this.equals(getJewishHolidayDate(9,this._y))||this.equals(getJewishHolidayDate(11,this._y))||this.equals(getJewishHolidayDate(17,this._y))
		||this.equals(getJewishHolidayDate(26,this._y))||this.equals(getJewishHolidayDate(42,this._y)))
	return true;
	if(isDiaspora()&&(this.equals(getJewishHolidayDate(3,this._y))||this.equals(getJewishHolidayDate(10,this._y))||this.equals(getJewishHolidayDate(12,this._y))
		||this.equals(getJewishHolidayDate(39,this._y))||this.equals(getJewishHolidayDate(40,this._y))))
	...
	for(i in cal._veses){
		...
		if(v._reeyah._y>y||(v._reeyah._y==y&&month_earlier_then(m,v._reeyah._m))){
			...
			if(v._reeyah._m==m&&v._reeyah._y==y){
				this._days[v._reeyah._d]._reeyah=v;if(v._onah==_NIGHT_)
...
{var d=new Date();var current_hdate=HDate_from_english(d.getDate(),d.getMonth(),d.getFullYear(),d.getHours(),d.getMinutes());if(date.is_later_then(current_hdate))
return false
